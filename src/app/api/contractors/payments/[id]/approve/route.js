...
export async function PUT(request, { params }) {
  try {
    ...
    // Generate success notification for cleared payment
    const notifTitle = `Payment Cleared`;
    const notifDesc = `Payment request of SAR ${Number(row.amount).toLocaleString()} for ${contractorRes.rows[0]?.name || "Unknown"} has been approved and cleared.`;
    ...
    try {
      ...
      console.error("Failed to generate payment approval notification:", nErr);
    }
